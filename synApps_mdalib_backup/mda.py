...
from . import f_xdrlib as xdr  # Always use fallback
...
class scanDetector:
    def __init__(self):
        ...
        self.data = (
            []
        )  # list of values read from 'name' PV.  If rank==2, lists of lists, etc.
...
def skimScan(dataFile):
    ...
    if (scan.rank > 20) or (scan.rank < 0):
        print(
            (
                "* * * skimScan('%s'): rank > 20.  probably a corrupt file"
                % dataFile.name
            )
        )
        return None
...
def writeMDA(dim, fname=None):
    ...
    # Write
    if fname == None:
        fname = tkinter.filedialog.SaveAs().show()
    f = open(fname, "wb")
...
def opMDA(op, d1, d2):
    ...
    for i in range(s[3].nd):
        for j in range(s[1].npts):
            for k in range(s[2].npts):
                s[3].d[i].data[j][k] = list(
                    map(op, s[3].d[i].data[j][k], d2[3].d[i].data[j][k])
                )
    ...
    for i in range(s[4].nd):
        for j in range(s[1].npts):
            for k in range(s[2].npts):
                for l in range(s[3].npts):
                    s[4].d[i].data[j][k][l] = list(
                        map(op, s[4].d[i].data[j][k][l], d2[4].d[i].data[j][k][l])
                    )
